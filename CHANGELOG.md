...
- Implements multi-pattern matching to `search_file()`. Loops through patterns and considers a line a match if ANY pattern matches before breaking after first match.- Added `patterns` parameter to `search_file()`, `search_multiple_files()`, and `search_directory_recursively()` to support multi-pattern matching.
...
- Updated matching logic: each line is considered a match if it matches any pattern in the provided list.
